...
const EMPTY_SECRETS: AppSecrets = { voiceToken: '', ai33Key: '', aimaxKey: '' };

export class SecretStore {
  private readonly filePath: string;

  constructor(userDataPath: string) {
    this.filePath = path.join(userDataPath, 'secrets.enc.json');
  }
  ...
  async getAll(): Promise<AppSecrets> {
    ...
    for (const key of Object.keys(result) as (keyof AppSecrets)[]) {
      ...
      try {
        ...
        // Ignore a value encrypted for another Windows account or machine.
      }
    }
    return result;
  }
  ...
  async merge(values: Partial<AppSecrets>): Promise<AppSecrets> {
    const supplied = Object.entries(values).filter((entry): entry is [keyof AppSecrets, string] => Boolean(entry[1]?.trim()));
    if (!supplied.length) return this.getAll();
    if (!safeStorage.isEncryptionAvailable()) {
      throw new Error('Windows không cung cấp kho mã hóa an toàn cho API key/token.');
    }
    const encrypted = await this.readEncrypted();
  ...
  async mergeMissing(values: Partial<AppSecrets>): Promise<AppSecrets> {
    const current = await this.getAll();
    const missing = Object.fromEntries(
      (Object.keys(current) as (keyof AppSecrets)[])
        .filter((key) => !current[key] && values[key])
        .map((key) => [key, values[key]]),
    ) as Partial<AppSecrets>;
    return this.merge(missing);
  }
}
