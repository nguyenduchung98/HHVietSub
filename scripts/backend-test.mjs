...
if (!python) throw new Error('Không tìm thấy Python. Hãy đặt biến DCC_PYTHON.');
const env = Object.fromEntries(Object.entries(process.env).filter(([key]) => key !== 'Path'));
const result = spawnSync(python, ['-m', 'unittest', 'discover', '-s', 'backend/tests', '-p', 'test_*.py'], { stdio: 'inherit', env });
