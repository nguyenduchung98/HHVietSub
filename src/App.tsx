...
function SrtVoicePage({ voices, initialDraft }: { voices: Voice[]; initialDraft: SrtDraft | null }) {
  ...
  const run = async (only?: SrtVoiceRow) => {
    ...
    if (!voiceId) return setMessage('Hãy chọn một hồ sơ giọng OmniVoice hoàn chỉnh.');
  ...
  return <div className="srt-voice-page">
    ...
    <section className="srt-voice-config"><label><small>FILE ĐẦU VÀO</small><strong>{draft?.name || 'Chưa chọn SRT'}</strong><span>{draft ? `${draft.rows.length} câu phụ đề` : 'Có thể nhận trực tiếp từ tab Dịch'}</span></label><label><small>GIỌNG OMNIVOICE</small><select value={voiceId} onChange={(e) => setVoiceId(e.target.value)}>{voices.filter((voice) => voice.ready).map((voice) => <option key={voice.id} value={voice.id}>{voice.name}</option>)}</select></label><label><small>TỐC ĐỘ · {speed.toFixed(2)}×</small><input type="range" min="0.6" max="1.5" step="0.05" value={speed} onChange={(e) => setSpeed(Number(e.target.value))} /></label><label><small>STEP / GUIDANCE</small><div className="inline-numbers"><input type="number" min="4" max="64" value={steps} onChange={(e) => setSteps(Number(e.target.value))} /><input type="number" min="0.5" max="5" step="0.1" value={guidance} onChange={(e) => setGuidance(Number(e.target.value))} /></div></label></section>
    ...
    <footer className="srt-voice-footer"><strong>{message}</strong><span>Retry lỗi: tối đa 2 lần</span></footer>{audioUrl && <audio className="srt-voice-player" src={audioUrl} controls />}
  </div>;
}
...
function VoiceBackendSettings() {
  ...
  return <div className="voice-backend-page"><div className="settings-hero"><div className="eyebrow"><Settings2 size={15}/> CẤU HÌNH ỨNG DỤNG</div><h1>Thiết lập cấu hình</h1><p>Chọn bộ xử lý OmniVoice cho Phòng thu và Tạo từ SRT.</p></div>
    <section className="voice-backend-card"><header><AudioLines size={23}/><div><h2>Cấu hình Bộ xử lý (Backend)</h2><p>Chạy trên máy hoặc dùng GPU Google Colab cho máy cấu hình yếu.</p></div></header>
      ...
      {mode === 'colab' && <div className="colab-config"><div><strong>Đường dẫn API Google Colab</strong><a href="https://colab.research.google.com/github/nguyenduchung98/HHVietSub-Colab/blob/main/HHVietSub_Colab.ipynb" target="_blank" rel="noreferrer">Mở Colab Notebook ↗</a></div><ol><li>Chọn Runtime → Change runtime type → GPU.</li><li>Run all và chờ URL dạng https://xxx.trycloudflare.com.</li><li>Dán URL, kiểm tra kết nối rồi lưu.</li></ol><input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://xxx.trycloudflare.com"/><input value={token} onChange={(e) => setToken(e.target.value)} placeholder="Token bảo mật (notebook mới, tùy chọn)"/><button onClick={test} disabled={testing}>{testing ? 'Đang kiểm tra…' : 'Kiểm tra kết nối'}</button></div>}
      <footer><span>{message}</span><button className="primary" onClick={save}>Lưu cấu hình</button></footer></section></div>;
}
