...
function SrtVoicePage({ voices, initialDraft }: { voices: Voice[]; initialDraft: SrtDraft | null }) {
  ...
  const run = async (only?: SrtVoiceRow) => {
    ...
    try {
      const result = await window.desktop.request<{ completed: number; failed: number; total: number; items: SrtVoiceRow[]; outputDir: string }>(only ? 'srt.voice.regenerate' : 'srt.voice.generate', {
        ...
        engine, apiProvider, apiModel, apiVoiceId: apiVoiceId.trim(), language: 'vi', speed, steps, guidance, postprocess: true, denoise: false, skipExisting: !only,
      });
  ...
  return <div className="srt-voice-page">
    ...
    <section className="srt-voice-config">
      ...
      {engine === 'omnivoice' ? <><label><small>GIỌNG OMNIVOICE</small><select value={voiceId} onChange={(e) => setVoiceId(e.target.value)}>{voices.filter((voice) => voice.ready).map((voice) => <option key={voice.id} value={voice.id}>{voice.name}</option>)}</select></label><label><small>STEP / GUIDANCE</small><div className="inline-numbers"><input type="number" min="4" max="64" value={steps} onChange={(e) => setSteps(Number(e.target.value))} /><input type="number" min="0.5" max="5" step="0.1" value={guidance} onChange={(e) => setGuidance(Number(e.target.value))} /></div></label></> : <><label><small>VOICE ID</small><input value={apiVoiceId} onChange={(e) => setApiVoiceId(e.target.value)} placeholder={engine === 'ai33' ? 'minimax_… / elevenlabs_…' : 'Voice ID thư viện'} /></label>{engine === 'aimax' ? <label><small>NHÀ CUNG CẤP / MODEL</small><div className="inline-numbers"><select value={apiProvider} onChange={(e) => { const p=e.target.value; setApiProvider(p); setApiModel(p === 'minimax' ? 'speech-2.8-hd' : 'eleven_multilingual_v2'); }}><option value="minimax">MiniMax</option><option value="elevenlabs">EL Voice</option></select><select value={apiModel} onChange={(e) => setApiModel(e.target.value)}>{apiProvider === 'minimax' ? <><option value="speech-2.8-hd">2.8 HD</option><option value="speech-2.8-turbo">2.8 Turbo</option><option value="speech-2.6-hd">2.6 HD</option><option value="speech-2.6-turbo">2.6 Turbo</option><option value="speech-02-hd">02 HD</option><option value="speech-02-turbo">02 Turbo</option></> : <><option value="eleven_v3">Eleven v3</option><option value="eleven_multilingual_v2">Multilingual v2</option><option value="eleven_flash_v2_5">Flash v2.5</option><option value="eleven_turbo_v2_5">Turbo v2.5</option></>}</select></div></label> : <label><small>NGUỒN GIỌNG AI33</small><span>Provider được nhận diện từ tiền tố Voice ID.</span><strong>AI33 Voice Library</strong></label>}</>}
